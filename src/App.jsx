      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="w-full flex justify-end items-center px-8 py-2 bg-transparent">
          <div className="flex gap-4 text-xs font-semibold">
            <span className={`inline-flex items-center gap-1 ${isWebSocketConnected ? 'text-green-600' : 'text-red-500'}`}>
              <span className={`w-2 h-2 rounded-full ${isWebSocketConnected ? 'bg-green-400' : 'bg-red-400'}`}></span>
              WebSocket: {isWebSocketConnected ? 'Connected' : 'Disconnected'}
            </span>
            <span className={`inline-flex items-center gap-1 ${isWebSocketConnected ? 'text-green-600' : 'text-red-500'}`}>
              <span className={`w-2 h-2 rounded-full ${isWebSocketConnected ? 'bg-green-400' : 'bg-red-400'}`}></span>
              MQTT: {isWebSocketConnected ? 'Connected' : 'Disconnected'}
            </span>
          </div>
        </div>
